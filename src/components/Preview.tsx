...
const Preview: React.FC<PreviewProps> = ({ selectedItem, isPreviewOpen, closePreview }) => {
    ...
    return (
        <>
            ...
            <div 
                className={`flex flex-col self-center justify-center fixed top-0 right-0 h-full w-[45%] bg-[#f0eeeb] shadow-lg transform transition-transform duration-300 z-50 ${isPreviewOpen ? 'translate-x-0' : 'translate-x-full'}`}>
                ...
                <hr />
                <div onClick={() => handleAction('cart', selectedItem)} className="hover:bg-gray-200 transition duration-300 ease-linear w-full flex justify-center flex-row items-center self-center p-5 cursor-pointer gap-4">
                    <PiBag size={30} />
